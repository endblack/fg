	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá 🤭 @${num.split('@')[0]}\nBem vindo ao grupo *${mdata.subject}* por favor não seja um ghost 😳 `
				...
				teks = `Tchau @${num.split('@')[0]} já foi tarde zé mané`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
				...
				case 'wikien':
					...
					break				
				case 'ytmp3':
					if (args.length < 1) return reply('Onde está o url tio?')
					...
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, audio, {mimetype: 'audio/mp4', Nome do arquivo: `${anu.title}.mp3`, quoted: mek})
					break
					case 'ytmp4':
				if (args.length < 1) return reply('Onde está o link do you tube mano?')
					tels = body.slice(7)				
					reply(mess.wait)
					buffer = await getBuffer(anu.thumb)
				 	anu = await fetchJson(`https://tobz-api.herokuapp.com/api/ytv?url=${tels}&apikey=BotWeA`, {method: 'get'})
                                        /*if (!isDaftar) return reply(mess.only.daftarB)*/
					hasil = `*❏ Título* : ${anu.title}\n*❏ Tamanho* : ${anu.filesize}\n*❏ resolução* : ${anu.resolution}\n*❏ Tipo* : ${anu.ext}\n*❏ Link* : ${anu.result}`					
				...
				case 'nulis': 
				case 'tulis': ini
					if (args.length < 1) return reply('Eu disse para você escrever, mano?')
                                        /*if (!isDaftar) return reply(mess.only.daftarB)*/
					teks = body.slice(7)
					reply(mess.wait)
	      ...
	      tels = body.slice(6)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
                ...
                client.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', nome: `${data.result.title}.mp3`, quoted: mek})
                break
				case 'info':
					me = client.user
					uptime = process.uptime()
					teks = `*Nome do bot* : ${me.name}\n*Número do bot* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Total de block* : ${blocked.length}\n*O bot está On agora* : ${kyun(uptime)}\n*Total Chat* : ${totalchat.length}`
				...
				case 'setppbot':
				client.updatePresence(from, Presence.composing) 
				if (!isQuotedImage) return reply(`Envie a imagem com a legenda ${prefix}setbotpp ou a tag da imagem já enviada`)
					if (!isOwner) return reply(mess.only.ownerB)
			...
			case 'simi':
				...
				//if (anu.error) return reply('Simi não sabe mano😕')
				reply(anu)
				break
			case 'simih':
				if (!isGroup) return reply(mess.only.group)
				...
				if (Number(args[0]) === 1) {
					...
					reply('1 para ativar, 0 para desativar, bobão vc hein')
				}
				break
				case 'alay':
                    client.updatePresence(from, Presence.composing) 
				...
				case 'chord':
					...
					break
				case 'lirik':
					if (args.length < 1) return reply('Onde estão as letras, mana?')
                                        /*if (!isDaftar) return reply(mess.only.daftarB)*/
					tels = body.slice(7)
           ...
           break
				...
				case 'add':
					...
					try {
						...
						console.log('Error :', e)
						reply('Falha ao adicionar membro, talvez porque seja privado')
					}
					break
				case 'kick':
					client.updatePresence(from, Presence.composing) 
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Falta marca o cara para da ban!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
				...
				case 'ocr':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply('Somente com fotos senhor')
					}
					break
	    ...
	    case 'kapankah':
			...
			case 'closegc':
				...
				case 'toimg':
				    client.updatePresence(from, Presence.composing)
                                    /*if (!isDaftar) return reply(mess.only.daftarB)*/
					if (!isQuotedSticker) return reply('❌ Só funfa com figu normal senhora ❌')
					reply(mess.wait)
                	...
                	client.updatePresence(from, Presence.composing) 
                        /*if (!isDaftar) return reply(mess.only.daftarB)*/
					if (!isQuotedVideo) return reply('❌ teste com vídeos man ❌')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
               ...
               if (anu.error) return reply(anu.error)
                ...
                client.sendMessage(from, buffer, image, {quoted: mek, caption: infomp3})
                client.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', nome do arquivo: `${anu.title}.mp3`, quoted: mek})
                break
                      case 'joox2':
                reply(mess.wait)
               ...
               if (data.error) return reply(data.error)
                ...
                lagu = await getBuffer(data.result.mp3)
                client.sendMessage(from, buffer, image, {quoted: mek, caption: infomp3})
                client.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', nome do arquivo : `${data.result.title}.mp3`, quoted: mek})
                break
                     case 'infocuaca':
            ...
            break
					...
					if (Number(args[0]) === 1) {
						...
						reply('digite 1 para ativar, 0 para desativar o recurso, bobão vc hein')
					}
                                        break
                                case 'fakta':
					fakta = body.slice(1)
